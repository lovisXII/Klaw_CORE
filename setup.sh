...
if ! command -v riscv32-unknown-elf-gcc &> /dev/null; then
    ...
    if [ -f "riscv32-unknown-elf.gcc-10.2.0.rv32i.ilp32.newlib.tar.gz" ] && [ "$(sha1sum riscv32-unknown-elf.gcc-10.2.0.rv32i.ilp32.newlib.tar.gz | cut -d ' ' -f 1)" != "$riscv_sha1" ]; then
    ...
    elif ! [ -f "riscv32-unknown-elf.gcc-10.2.0.rv32i.ilp32.newlib.tar.gz" ]; then
        wget https://github.com/stnolting/riscv-gcc-prebuilt/releases/download/rv32i-2.0.0/riscv32-unknown-elf.gcc-10.2.0.rv32i.ilp32.newlib.tar.gz
    fi
...
cd $setup_path && rm -rf riscv32-unknown-elf.gcc-10.2.0.rv32i.ilp32.newlib.tar.gz systemc-2.3.3.gz
